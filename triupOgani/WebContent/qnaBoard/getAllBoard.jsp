...
<body>
		...
		<div class="container">
			...
			<div align="center">
				<table width="700" bgcolor="<%=value_c%>" border="1">
					<tr>
						<td style="border-right: hidden;"><b>고객문의 게시판(전체 글:<%=count%>)
						</b></td>
						<td align="right"><a href="index.jsp">홈으로 이동</a> <a
							href="qnaBoard/writeForm.jsp">글쓰기</a></td>
				</table>
 ...
 %> <img
							...
							%> <a
							href="qnaController?choice=get-board&num=<%=board.getNum()%>&pageNum=<%=pageNum%>">
								<%=board.getSubject()%></a> <%
 ...
 %> <img
				...
				%>
				<a
					href="qnaController?choice=getAll-board&pageNum=<%=startPage - 10%>&startRow=<%=(startPage - 1) * 10 + 1%>&endRow=<%=startPage * 10%>">[이전]</a>
				<%
					}
						for (int i = startPage; i <= endPage; i++) {
				%>
				<a
					href="qnaController?choice=getAll-board&pageNum=<%=i%>&startRow=<%=(i - 1) * 10 + 1%>&endRow=<%=i * 10%>">[<%=i%>]
				</a>
				<%
					}
						if (endPage < pageCount) {
				%>
				<a
					href="qnaController?choice=getAll-board&pageNum=<%=startPage + 10%>&startRow=<%=(startPage - 1) * 10 + 1%>&endRow=<%=startPage * 10%>">[다음]</a>
				<%
					}
					}
				%>
